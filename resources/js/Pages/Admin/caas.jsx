...
import {
    ...
    ArrowDownTrayIcon, ArrowUpTrayIcon, MagnifyingGlassIcon,
    ListBulletIcon, TableCellsIcon, UserGroupIcon,
            ...
            <div className="relative w-full max-w-2xl bg-[#0a121d] border-2 border-double border-cyan-600/30 p-10 shadow-2xl animate-popIn">
                ...
                <form onSubmit={onSubmit} className="flex flex-col gap-6 text-sm text-white">
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            <label className="text-cyan-500/60 text-[10px] font-bold uppercase tracking-widest block mb-2">Subject Name</label>
                            <input type="text" required className="input-etched w-full" value={form.data.name} onChange={(e) => form.setData("name", e.target.value)} />
                        </div>
                        <div>
                            <label className="text-cyan-500/60 text-[10px] font-bold uppercase tracking-widest block mb-2">NIM</label>
                            <input type="text" required disabled={form.data.id} className={`input-etched w-full ${form.data.id ? "opacity-50 cursor-not-allowed" : ""}`} value={form.data.nim} onChange={(e) => form.setData("nim", e.target.value)} />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 gap-6">
                        <div>
                            <label className="text-cyan-500/60 text-[10px] font-bold uppercase tracking-widest block">Gender</label>
                            <select required className="input-etched w-full" value={form.data.gender} onChange={(e) => form.setData("gender", e.target.value)}>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            <label className="text-cyan-500/60 text-[10px] font-bold uppercase tracking-widest block mb-2">Major</label>
                            <select required className="input-etched w-full" value={form.data.major} onChange={(e) => form.setData("major", e.target.value)}>
                        ...
                        <div>
                            <label className="text-cyan-500/60 text-[10px] font-bold uppercase tracking-widest block mb-2">Assigned Class</label>
                            <input type="text" required className="input-etched w-full font-mono" value={form.data.class} onChange={(e) => form.setData("class", e.target.value)} />
                        </div>
                    </div>
                    <button type="submit" disabled={form.processing} className={`w-full py-4 border font-bold font-serif uppercase tracking-[0.2em] transition-all ${form.processing ? "bg-gray-600/20 border-gray-500/30 text-gray-400 cursor-not-allowed" : "bg-cyan-700/20 border-cyan-500/50 text-cyan-100 hover:bg-cyan-600 hover:text-black"}`}>
                        {form.processing ? (form.data.id ? "Updating..." : "Creating...") : form.data.id ? "Update Record" : "Authorize Record"}
                    </button>
                </form>
            </div>
        </div>,
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-[#0a121d] border-2 border-double border-cyan-600/30 shadow-2xl animate-popIn flex flex-col overflow-hidden">
                <div className="p-8 border-b border-white/5 bg-[#050a10]">
                    ...
                    <p className="text-white/40 text-[10px] font-mono uppercase text-center mt-1">Unique Record Identifier: {viewData.nim}</p>
                </div>
                <div className="p-10 flex flex-col gap-8 text-sm text-white">
                    <div className="grid grid-cols-2 gap-8">
                        <div>
                            <span className="block text-cyan-500/50 text-[10px] font-bold uppercase mb-1 tracking-widest">Subject Identity</span>
                            <span className="text-2xl font-serif">{viewData.profile?.name || "N/A"}</span>
                        </div>
                        <div>
                            <span className="block text-cyan-500/50 text-[10px] font-bold uppercase mb-1 tracking-widest">NIM / ID</span>
                            <span className="text-lg font-mono text-cyan-200">{viewData.nim}</span>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-8 pt-6 border-t border-white/5">
                        <div>
                            <span className="block text-cyan-500/50 text-[10px] font-bold uppercase mb-1 tracking-widest">Major</span>
                            <span className="text-base font-bold text-cyan-100">{viewData.profile?.major || "N/A"}</span>
                        </div>
                        <div>
                            <span className="block text-cyan-500/50 text-[10px] font-bold uppercase mb-1 tracking-widest">Class Code</span>
                            <span className="text-base font-bold font-mono text-cyan-100">{viewData.profile?.class || "N/A"}</span>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-8 pt-6 border-t border-white/5">
                        <div>
                            <span className="block text-cyan-500/50 text-[10px] font-bold uppercase mb-1 tracking-widest">Caas Stage</span>
                            <span className="text-cyan-200 font-bold font-mono tracking-wider">{viewData.caas_stage?.stage?.name || "N/A"}</span>
                        </div>
                        <div>
                            <span className="block text-cyan-500/50 text-[10px] font-bold uppercase mb-2 tracking-widest">Gender</span>
                ...
                <div className="p-6 border-t border-white/5 bg-[#050a10] flex justify-center">
                    <button onClick={onClose} className="px-14 py-4 bg-cyan-900/20 border border-white/10 text-white/60 hover:text-white transition-all text-xs font-serif font-bold uppercase tracking-[0.3em]">
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#0a121d] border-2 border-double border-amber-600/30 p-12 max-w-md w-full text-center animate-popIn shadow-2xl text-white">
                ...
                <div className="flex gap-6">
                    ...
                    <button onClick={onConfirm} className="flex-1 py-3 font-bold text-white text-xs uppercase tracking-widest shadow-lg bg-rose-700/20 border border-rose-500/50 hover:bg-rose-600 hover:text-black transition-all">
                        Confirm
                    </button>
                </div>
            </div>
...
export default function Caas({ users, stages, filters }) {
    ...
    // --- Derived Data ---
    const totalCandidatesCount = useMemo(() => users.data.reduce((acc, curr) => acc + (curr.plottingans?.length || 0), 0), [users]);
    ...
    const handleFileChange = (e) => {
        ...
        router.post("/admin/caas/import", formData, {
            ...
            onError: (errors) => {
                const errorMsg = errors.file || errors.error || Object.values(errors)[0] || "Import failed. Please check your file format.";
                setConfirmModal({
                    isOpen: true,
                    title: "Import Failed",
                    message: errorMsg,
    ...
    return (
        <>
            ...
            <div className="fixed inset-0 w-full h-full bg-[#0a2a4a] text-white overflow-y-auto md:overflow-hidden font-sans">
                ...
                <div className={`relative md:absolute md:inset-0 z-10 flex flex-col items-center justify-start md:justify-center p-4 md:p-8 transition-all duration-1000 ${isZooming ? "opacity-0 scale-95" : "opacity-100 scale-100"}`}>
                    ...
                    <div className="text-center relative z-10 mb-8 mt-24 md:mt-0 w-full max-w-7xl flex flex-col md:flex-row items-center md:items-end justify-between gap-6">
                        ...
                        <div className="flex gap-4 w-full md:w-auto">
                            <StatCard label="Total Subjects" value={totalUsers} type="total" />
                        </div>
                    </div>
                        ...
                        <div className="atlantean-panel flex flex-col border-t-0 rounded-b-2xl overflow-hidden">
                                    ...
                                    <tbody className="font-sans">
                                        {users.data.map((item, index) => (
                                            <tr key={item.id} className="border-b border-white/5 hover:bg-cyan-400/5 transition-colors group">
                                                <td className={`${viewMode === "small" ? "px-4 py-1.5 text-sm" : "p-4 pl-8 text-base"} font-mono text-white/30`}>
                                                    {(currentPage - 1) * ITEMS_PER_PAGE + index + 1}
                                                </td>
                                                <td className={`${viewMode === "small" ? "px-4 py-1.5" : "p-4"}`}>
                                                    <div className="flex flex-col">
                                                        <span className={`${viewMode === "small" ? "text-sm" : "text-base"} font-bold uppercase tracking-wider text-white`}>
                                                ...
                                                <td className={`${viewMode === "small" ? "px-4 py-1.5 text-sm" : "p-4 text-base"} font-bold text-cyan-100/90`}>
                                                    {item.profile?.major || "N/A"}
                                                </td>
                                                <td className={`${viewMode === "small" ? "px-4 py-1.5 text-sm" : "p-4 text-base"} font-bold text-cyan-100`}>
                                                    {item.profile?.class || "N/A"}
                                                </td>
                                                <td className={`${viewMode === "small" ? "px-4 py-1.5" : "p-4"}`}>
                                                    <select
                                                        value={item.caas_stage?.stage_id || ""}
                                                        onChange={(e) => handleStageChange(item.caas_stage?.id, e.target.value)}
                                                ...
                                                <td className={`${viewMode === "small" ? "px-4 py-1.5" : "p-4"} pr-8`}>
                                                    <div className="flex justify-center gap-3">
                                                        ...
                                                        <button
                                                            onClick={() => handleDelete(item.id, item.profile?.name, item.nim)}
                            ...
                            <div className="p-4 border-t border-white/5 bg-[#0f1c2e]/80 backdrop-blur-md flex justify-between items-center text-xs font-serif">
                                ...
                                <div className="flex gap-4">
                                    <button
                                        onClick={() => router.get("/admin/caas", { page: currentPage - 1, perPage: ITEMS_PER_PAGE, search: searchQuery }, { preserveState: true })}
                ...
                {/* Modals Extracted */}
                <CaasFormModal isOpen={isFormOpen} form={form} onSubmit={handleSave} onClose={closeAllModals} />
                <CaasViewModal isOpen={isViewOpen} viewData={viewData} onClose={closeAllModals} />
