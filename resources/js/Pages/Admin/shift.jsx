...
export default function Shift({ shifts }) {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e) => {
            if (e.key === "Escape") {
                if (isFormOpen || isPlotterOpen || confirmModal.isOpen)
                    closeAllModals();
                else skipIntro();
            }
        };
    ...
    const totalCandidatesCount = useMemo(
        () =>
            shifts.data.reduce((acc, curr) => acc + curr.plottingans.length, 0),
        [shifts],
    );
    ...
    const handleExportExcel = () => {
        const exportData = shifts.data.map((s) => ({
            Shift: s.shift_no,
            Date: s.date,
            Start: s.time_start,
            End: s.time_end,
            Booked: s.plottingans?.length || 0,
            Quota: s.kuota,
        }));
                                    ...
                                    <tbody className="font-sans">
                                        {shifts.data.map((item, index) => (
                                            ...
                                            >
                                                ...
                                                <td
                                                    className={`${viewMode === "compact" ? "py-1.5 text-sm" : "p-4 text-base md:text-lg"} font-bold uppercase tracking-widest`}
                                                >
                                                    {item.shift_no}
                                                </td>
                                                <td
                                                    className={`${viewMode === "compact" ? "py-1.5 text-[11px]" : "p-4 text-sm"} font-mono`}
                                                >
                                                ...
                                                {/* <td
                                                    className={`${viewMode === "compact" ? "py-1.5" : "p-4"}`}
                                                >
                                                    <div className="flex flex-wrap items-center gap-2">
                                                        {(item.guardians || [])
                                                            ...
                                                            .map((g, i) => (
                                                                <span
                                                                    key={i}
                                                                    className={`px-2.5 py-1 bg-cyan-900/40 border border-cyan-500/30 rounded-sm font-bold text-cyan-100 ${viewMode === "compact" ? "text-[9px]" : "text-[11px]"}`}
                                                                >
                                                                    {g}
                                                                </span>
                                                            ))}
                                                        {(item.guardians || [])
                                                            .length > 5 && (
                                                            <span className="text-cyan-400/70 font-mono italic font-bold ml-1 text-[10px]">
                                                                ... +
                                                                {(
                                                                    item.guardians ||
                                                                    []
                        ...
                        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                            ...
                            <div className="relative w-full max-w-xl bg-[#0a121d] border-2 border-double border-cyan-600/30 p-10 shadow-2xl animate-popIn">
                                ...
                                >
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <label className="text-cyan-500/60 font-bold uppercase tracking-widest text-[10px]"></label>
                                        <input
                                            type="date"
                                            required
                                            className="input-etched w-full"
                                ...
                                <div className="flex-1 flex flex-col overflow-hidden text-white">
                                    <div className="w-full p-10 flex flex-col gap-8 overflow-y-auto custom-scrollbar">
                                        ...
                                        <div className="flex flex-col gap-4">
                                            {(
                                                activeShift.plottingans || []
                                            ).map((plotting) => (
                                                <div
                                                    key={plotting.id}
                                                    className="p-5 bg-emerald-950/10 border border-white/5 flex justify-between items-center rounded-sm transition-all group"
                                                >
                                                    <div className="flex flex-col">
