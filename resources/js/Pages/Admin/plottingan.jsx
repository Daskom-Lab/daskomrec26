...
export default function Plottingan({ shifts }) {
    ...
    // Fetch all users for a shift from API
    ...
    const filteredData = shifts.data.filter((item) => {
        if (!searchQuery) return true;
        const query = searchQuery.toLowerCase();
        const shiftNo = item.shift_no?.toLowerCase() || "";
        return shiftNo.includes(query);
    });
                                ...
                                <table className="w-full text-left border-collapse min-w-[800px]">
                                    ...
                                    <tbody className="font-sans">
                                        {filteredData.map((item, index) => (
                                            ...
                                            >
                                                <td
                                                    className={`${viewMode === "compact" ? "py-1.5" : "p-4"} pl-8 font-mono text-white/30 text-[10px]`}
                                                >
                                                    {(currentPage - 1) *
                                                        shifts.per_page +
                                                        index +
                                                ...
                                                <td
                                                    className={`${viewMode === "compact" ? "py-1.5 text-sm" : "p-4 text-base"} font-bold uppercase tracking-widest`}
                                                >
                                                    {item.shift_no || "-"}
                                                </td>
                                                <td
                                                    className={`${viewMode === "compact" ? "py-1.5 text-[11px]" : "p-4 text-sm"} font-mono text-cyan-100`}
                                                >
                                                    {formatDisplayDate(
                                                        item.date,
                                                    )}
                                                ...
                                                <td
                                                    className={`${viewMode === "compact" ? "py-1.5 text-[11px]" : "p-4 text-sm"} font-mono text-white/60`}
                                                >
                                                    {formatDisplayTime(
                                                        item.time_start,
                                                    )}{" "}
                                                ...
                                                <td
                                                    className={`${viewMode === "compact" ? "py-1.5 text-[11px]" : "p-4 text-sm"} font-medium`}
                                                >
                                                ...
                                                <td
                                                    className={`${viewMode === "compact" ? "py-1.5" : "p-4"} pr-8`}
                                                >
                            ...
                            <div className="relative w-full max-w-2xl bg-[#0a121d] border-2 border-double border-cyan-600/30 shadow-2xl animate-popIn flex flex-col max-h-[85vh] overflow-hidden">
                                <div className="p-8 border-b border-white/5 bg-[#050a10] flex justify-between items-center">
                                    <div className="flex flex-col">
                                        ...
                                        <h2 className="text-3xl font-serif font-bold text-cyan-100 tracking-widest uppercase">
                                ...
                                <div className="flex-1 flex flex-col overflow-hidden text-white">
                                    <div className="w-full p-8 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
                                        ...
                                        {loadingShiftUsers ? (
                                            ...
                                            <div className="flex flex-col gap-3">
                                                {shiftUsers.map((plotting) => (
                                                    <div
                                                        key={plotting.id}
                                                        className="p-4 bg-emerald-950/10 border border-white/5 flex justify-between items-center rounded-sm transition-all group hover:border-emerald-500/20"
                                                    >
                                                        <div className="flex flex-col">
                                ...
                                <div className="p-4 bg-[#050a10] border-t border-white/5 flex justify-end">
                                    <button
                                        onClick={closeAllModals}
                                        className="px-10 py-3 bg-cyan-900/20 border border-white/10 text-white/60 hover:text-white hover:border-white/30 transition-all text-xs font-serif font-bold uppercase tracking-[0.3em]"
                                    >
                                        Close
                                    </button>
                                </div>
