...
const ShiftTable = ({ shifts = [], onAddShift, hasChosen = false }) => {
        ...
        if (sortConfig.key) {
            data.sort((a, b) => {
                ...
                if (sortConfig.key === "availability") {
                    const aLeft =
                        (a.quota || 0) -
                        (a.caasBooked ? a.caasBooked.length : 0);
                    const bLeft =
                        (b.quota || 0) -
                        (b.caasBooked ? b.caasBooked.length : 0);
    ...
    const totalPages = Math.max(
        1,
        Math.ceil(processedShifts.length / ITEMS_PER_PAGE),
    );
        ...
        <div className="w-full max-w-[1200px] mx-auto font-sans text-white">
            ...
            <div className="atlantean-panel p-4 md:p-6 flex flex-col md:flex-row justify-between items-center gap-6 rounded-t-2xl shadow-[0_0_30px_rgba(8,145,178,0.15)]">
                <div className="flex items-center gap-4">
                    ...
                    <div className="ml-4">
                        <h1 className="text-xl sm:text-2xl font-serif font-bold text-white uppercase tracking-widest text-shadow">
                            Choose wisely
                        </h1>
                        <h1 className="text-sm text-red-400/70 font-bold uppercase tracking-[0.2em]">
                            It will be an irreversible process
                        </h1>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[900px]">
                        ...
                        <tbody className="font-sans">
                            {paginatedShifts.map((item, index) => {
                                const bookedCount = item.caasBooked
                                ...
                                return (
                                    ...
                                    >
                                        <td
                                            className={`${viewMode === "compact" ? "py-2" : "p-4"} pl-6 font-mono text-white/30 text-[10px]`}
                                        >
                                            {(currentPage - 1) *
                                                ITEMS_PER_PAGE +
                                                index +
                                        ...
                                        <td
                                            className={`${viewMode === "compact" ? "py-2 text-sm" : "p-4 text-base"} font-bold uppercase tracking-widest text-white`}
                                        >
                                            {item.shift}
                                        </td>

                                        <td
                                            className={`${viewMode === "compact" ? "py-2" : "p-4"} text-sm font-mono text-cyan-100`}
                                        >
                                            <div className="flex items-center gap-2">
                                        ...
                                        <td
                                            className={`${viewMode === "compact" ? "py-2" : "p-4"} text-sm font-mono text-white/80`}
                                        >
                                            <div className="flex items-center gap-2">
                                        ...
                                        <td
                                            className={`${viewMode === "compact" ? "py-2" : "p-4"}`}
                                        >
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-baseline gap-2">
                                                    <span
                                                        className={`text-lg font-bold ${isFull ? "text-rose-400" : "text-emerald-400"}`}
                                                    >
                                                        {available}
                                                    </span>
                                                ...
                                                <div className="w-20 h-1 bg-white/10 rounded-full overflow-hidden">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${Math.max(0, availabilityPercent)}%`,
                                                        }}
                                                    />
                                                </div>
                                            </div>
                                        ...
                                        <td
                                            className={`${viewMode === "compact" ? "py-2" : "p-4"} pr-6 text-center`}
                                        >
                                            <button
                                                onClick={() =>
                                                    !isFull &&
                                                ...
                                                disabled={isFull || hasChosen}
                                                className={`
                                                    group relative inline-flex items-center justify-center gap-2
                                                    ${viewMode === "compact" ? "px-4 py-1.5" : "px-6 py-2.5"}
                                                    ${isFull || hasChosen ? "opacity-50 cursor-not-allowed bg-slate-800 border-slate-700" : "bg-cyan-600/20 hover:bg-cyan-500 border-cyan-500/50 hover:border-cyan-400"}
                                                    border border-double rounded-sm transition-all duration-300 shadow-lg
                                                `}
                                            >
                                                {hasChosen ? (
                                                    <span className="text-[10px] font-serif font-bold uppercase tracking-widest text-amber-300/70">
                                                        Chosen
                                                    </span>
                                                ) : isFull ? (
                                                    <span className="text-[10px] font-serif font-bold uppercase tracking-widest text-white/40">
                                                        Closed
                                                    </span>
                                                ) : (
                                                    <>
                                                        <UserPlusIcon className="w-4 h-4 text-cyan-200 group-hover:text-white" />
                                                        <span className="text-[10px] font-serif font-bold uppercase tracking-widest text-cyan-100 group-hover:text-white">
                                                            Add
                                                        </span>
                                                    </>
                                                )}
                ...
                <div className="p-4 border-t border-white/5 bg-[#0f1c2e]/80 backdrop-blur-md flex justify-between items-center text-xs text-white">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="p-2 border border-white/10 text-cyan-100 hover:bg-cyan-500/20 disabled:opacity-20 transition-all rounded-sm"
                        >
